...
public class Deal {
    ...
    public Deal(UUID leadId, BigDecimal amount) {
        ...
        this.leadId = Objects.requireNonNull(leadId, "leadId не должен быть null");
        this.amount = Objects.requireNonNull(amount, "amount не должен быть null");
    ...
    public void transitionTo(DealStatus newStatus) {
        if (status.canTransitionTo(newStatus)) {
            ...
            throw new IllegalStateException("Невозможно сменить статус с " + status + " на " + newStatus);
        }
    }
