...
class LeadRepositoryJPQLTest {
  ...
  void shouldShowLeads_WithCurrentEmail_AndCompany() {
    ...
    assertThat(repository.findByStatusAndCompany(LeadStatus.NEW, firstCompany)).hasSize(1);

    // Валидное название фирмы, но не статус
    assertThat(repository.findByStatusAndCompany(LeadStatus.QUALIFIED, firstCompany)).isEmpty();

    // Валидный статус, но не название фирмы
    assertThat(repository.findByStatusAndCompany(LeadStatus.NEW, secondCompany)).isEmpty();
  }
