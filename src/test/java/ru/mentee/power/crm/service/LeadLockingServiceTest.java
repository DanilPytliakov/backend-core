...
class LeadLockingServiceTest {
  ...
  void shouldRetryTransactionwhenConcurrentUpdateWithOptimisticLock() throws Exception {
    ...
    assertThat(result1.lead().getStatus()).isIn(LeadStatus.CONTACTED, LeadStatus.QUALIFIED);

    assertThat(result2.lead().getStatus()).isIn(LeadStatus.CONTACTED, LeadStatus.QUALIFIED);

    // 2. Хоть у одной был retry
    assertThat(result1.attempts() > 1 || result2.attempts() > 1).isTrue();
